...
APP_CONFIG_SECRET_ID = os.environ.get(
...
def load_app_config():
    secret_value = secretsmanager.get_secret_value(
        SecretId=APP_CONFIG_SECRET_ID
    )
    config = json.loads(secret_value["SecretString"])

    # Do not print secret values. Only print safe metadata.
    print(
        "Loaded app config from Secrets Manager. "
        f"Keys available: {list(config.keys())}"
    )
...
def lambda_handler(event, context):
    try:
        ...
        return response(
            ...
            {
                "message": (
                    "Event accepted, loaded config, saved to DynamoDB, "
                    "and published to Kinesis"
                ),
                "event": body,
            }
        )
